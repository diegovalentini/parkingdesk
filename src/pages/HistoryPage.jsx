...
export default function HistoryPage() {
  ...
  return <>
    ...
    <main className="layout history-layout">
      <section className="hero-card history-hero"><div><p className="eyebrow">Historial diario</p><h2>Movimientos de hoy</h2><p className="muted">Los usuarios pueden consultar el historial y corregir el método de pago cuando haga falta.</p></div><button className="secondary-btn" type="button" onClick={() => showToast('Historial actualizado.')}>Actualizar</button></section>
      <section className="history-toolbar"><label className="search-box"><span>Buscar por patente o usuario que cerró</span><input value={search} onChange={(e) => setSearch(e.target.value)} type="search" placeholder="Ej: AB123CD o Diego" autoComplete="off" /></label></section>
      <section className="history-card"><div className="section-head"><div><h3>Registros cerrados hoy</h3><p className="muted">Movimientos cerrados del día actual.</p></div></div><div className="history-list" aria-live="polite"><HistoryRows logs={filteredLogs} user={user} search={search} onEdit={setEditing} onDelete={deleteLog} /></div></section>
    </main>
