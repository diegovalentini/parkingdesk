import { A as getExperimentalSetting, B as isMobileCordova, E as deepEqual, F as isCloudWorkstation, G as pingServer, H as isReactNative, I as isCloudflareWorker, K as querystring, L as isEmpty, M as getModularInstance, N as getUA, O as extractQuerystring, P as isBrowserExtension, R as isIE, S as base64Decode, T as createSubscribe, V as isNode, _ as Logger, b as ErrorFactory, g as LogLevel, m as registerVersion, n as SDK_VERSION, o as _isFirebaseServerApp, q as querystringDecode, s as _registerComponent, v as Component, x as FirebaseError, y as Deferred, z as isIndexedDBAvailable } from "./index.esm-BlupPgNa.js";
import { t as firebase } from "./index.esm-BwSUAgjI.js";
//#region node_modules/@firebase/auth/dist/esm/index-d90d2ee5.js
/**
* @license
...
*/
function _isOnline() {
	if (typeof navigator !== "undefined" && navigator && "onLine" in navigator && typeof navigator.onLine === "boolean" && (_isHttpOrHttps$1() || isBrowserExtension() || "connection" in navigator)) return navigator.onLine;
	return true;
}
...
async function _reloadWithoutSaving(user) {
	...
	_assert$4(response?.users.length, auth, "internal-error");
...
var StsTokenManager = class StsTokenManager {
	...
	static fromJSON(appName, object) {
		...
		if (refreshToken) {
			_assert$4(typeof refreshToken === "string", "internal-error", { appName });
			manager.refreshToken = refreshToken;
		}
		if (accessToken) {
			_assert$4(typeof accessToken === "string", "internal-error", { appName });
			manager.accessToken = accessToken;
		}
		if (expirationTime) {
			_assert$4(typeof expirationTime === "number", "internal-error", { appName });
...
var UserImpl = class UserImpl {
	...
	static _fromJSON(auth, object) {
		...
		_assert$4(uid && plainObjectTokenManager, auth, "internal-error");
		const stsTokenManager = StsTokenManager.fromJSON(this.name, plainObjectTokenManager);
		_assert$4(typeof uid === "string", auth, "internal-error");
		assertStringOrUndefined(displayName, auth.name);
		assertStringOrUndefined(email, auth.name);
		_assert$4(typeof emailVerified === "boolean", auth, "internal-error");
		_assert$4(typeof isAnonymous === "boolean", auth, "internal-error");
	...
	*/
	static async _fromGetAccountInfoResponse(auth, response, idToken) {
		const coreAccount = response.users[0];
		_assert$4(coreAccount.localId !== void 0, "internal-error");
...
var AuthImpl = class {
	...
	*/
	...
	async updateCurrentUser(userExtern) {
		...
		if (user) _assert$4(user.auth.config.apiKey === this.config.apiKey, this, "invalid-user-token");
		return this._updateCurrentUser(user && user._clone(this));
	}
	async _updateCurrentUser(user, skipBeforeStateCallbacks = false) {
		if (this._deleted) return;
		if (user) _assert$4(this.tenantId === user.tenantId, this, "tenant-id-mismatch");
...
function connectAuthEmulator(auth, url, options) {
	...
	if (!authInternal._canInitEmulator) {
		_assert$4(authInternal.config.emulator && authInternal.emulatorConfig, authInternal, "emulator-config-failed");
		_assert$4(deepEqual(emulator, authInternal.config.emulator) && deepEqual(emulatorConfig, authInternal.emulatorConfig), authInternal, "emulator-config-failed");
		return;
	}
...
async function _reauthenticate(user, credential, bypassAuthState = false) {
	...
	try {
		...
		_assert$4(response.idToken, auth, "internal-error");
		const parsed = _parseToken(response.idToken);
		_assert$4(parsed, auth, "internal-error");
		const { sub: localId } = parsed;
		_assert$4(user.uid === localId, auth, "user-mismatch");
...
function _setActionCodeSettingsOnRequest(auth, request, actionCodeSettings) {
	_assert$4(actionCodeSettings.url?.length > 0, auth, "invalid-continue-uri");
	_assert$4(typeof actionCodeSettings.dynamicLinkDomain === "undefined" || actionCodeSettings.dynamicLinkDomain.length > 0, auth, "invalid-dynamic-link-domain");
	_assert$4(typeof actionCodeSettings.linkDomain === "undefined" || actionCodeSettings.linkDomain.length > 0, auth, "invalid-hosting-link-domain");
	request.continueUrl = actionCodeSettings.url;
	request.dynamicLinkDomain = actionCodeSettings.dynamicLinkDomain;
	request.linkDomain = actionCodeSettings.linkDomain;
	request.canHandleCodeInApp = actionCodeSettings.handleCodeInApp;
	if (actionCodeSettings.iOS) {
		_assert$4(actionCodeSettings.iOS.bundleId.length > 0, auth, "missing-ios-bundle-id");
		request.iOSBundleId = actionCodeSettings.iOS.bundleId;
	}
	if (actionCodeSettings.android) {
		_assert$4(actionCodeSettings.android.packageName.length > 0, auth, "missing-android-pkg-name");
...
async function checkActionCode(auth, oobCode) {
	...
	switch (operation) {
		...
		case "VERIFY_AND_CHANGE_EMAIL":
			...
			break;
		case "REVERT_SECOND_FACTOR_ADDITION": _assert$4(response.mfaInfo, authModular, "internal-error");
		default: _assert$4(response.email, authModular, "internal-error");
	}
...
async function sendSignInLinkToEmail(auth, email, actionCodeSettings) {
	...
	function setActionCodeSettings(request, actionCodeSettings) {
		_assert$4(actionCodeSettings.handleCodeInApp, authInternal, "argument-error");
		if (actionCodeSettings) _setActionCodeSettingsOnRequest(authInternal, request, actionCodeSettings);
	}
...
async function fetchSignInMethodsForEmail(auth, email) {
	const request = {
		identifier: email,
		continueUri: _isHttpOrHttps$1() ? _getCurrentUrl() : "http://localhost"
	};
...
var MultiFactorResolverImpl = class MultiFactorResolverImpl {
	...
	static _fromError(authExtern, error) {
		...
		_assert$4(serverResponse.mfaPendingCredential, auth, "internal-error");
		...
		return new MultiFactorResolverImpl(session, hints, async (assertion) => {
			...
			switch (error.operationType) {
				case "signIn":
					...
					return userCredential;
				case "reauthenticate":
					_assert$4(error.user, auth, "internal-error");
...
function getMultiFactorResolver(auth, error) {
	...
	_assert$4(errorInternal.customData._serverResponse?.mfaPendingCredential, authModular, "argument-error");
...
var ReCaptchaLoaderImpl = class {
	...
	load(auth, hl = "") {
		_assert$4(isHostLanguageValid(hl), auth, "argument-error");
	...
	try {
		...
		if ("session" in phoneInfoOptions) {
			...
			if ("phoneNumber" in phoneInfoOptions) {
				_assert$4(session.type === "enroll", auth, "internal-error");
				...
				const startEnrollPhoneMfaActionCallback = async (authInstance, request) => {
					if (request.phoneEnrollmentInfo.captchaResponse === FAKE_TOKEN) {
						_assert$4(verifier?.type === RECAPTCHA_VERIFIER_TYPE, authInstance, "argument-error");
				...
				_assert$4(session.type === "signin", auth, "internal-error");
				const mfaEnrollmentId = phoneInfoOptions.multiFactorHint?.uid || phoneInfoOptions.multiFactorUid;
				_assert$4(mfaEnrollmentId, auth, "missing-multi-factor-info");
...
async function injectRecaptchaV2Token(auth, request, recaptchaV2Verifier) {
	_assert$4(recaptchaV2Verifier.type === RECAPTCHA_VERIFIER_TYPE, auth, "argument-error");
	const recaptchaV2Token = await recaptchaV2Verifier.verify();
	_assert$4(typeof recaptchaV2Token === "string", auth, "argument-error");
	const newRequest = { ...request };
	if ("phoneEnrollmentInfo" in newRequest) {
		const phoneNumber = newRequest.phoneEnrollmentInfo.phoneNumber;
...
* @public
*/
var PhoneAuthProvider$1 = class PhoneAuthProvider$1 {
	/**
...
function _withDefaultResolver(auth, resolverOverride) {
	...
	_assert$4(auth._popupRedirectResolver, auth, "argument-error");
...
function _reauth(params) {
	...
	_assert$4(user, auth, "internal-error");
...
function getIframeUrl(auth) {
	...
	_assert$4(config.authDomain, auth, "auth-domain-config-required");
...
async function _openIframe(auth) {
	...
	_assert$4(gapi, auth, "internal-error");
	return context.open({
		where: document.body,
...
function _open(auth, url, name, width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT) {
	...
	_assert$4(newWin, auth, "popup-blocked");
	try {
		newWin.focus();
	} catch (e) {}
...
async function _getRedirectUrl(auth, provider, authType, redirectUrl, eventId, additionalParams) {
	_assert$4(auth.config.authDomain, auth, "auth-domain-config-required");
	_assert$4(auth.config.apiKey, auth, "invalid-api-key");
	const params = {
...
var BrowserPopupRedirectResolver = class {
	...
	async initAndGetManager(auth) {
		...
		iframe.register("authEvent", (iframeEvent) => {
			_assert$4(iframeEvent?.authEvent, auth, "invalid-auth-event");
	...
	_originValidation(auth) {
		const key = auth._key();
		if (!this.originValidationPromises[key]) this.originValidationPromises[key] = _validateOrigin$1(auth);
...
var TotpMultiFactorGenerator = class {
	...
	*/
	static async generateSecret(session) {
		...
		_assert$4(typeof mfaSession.user?.auth !== "undefined", "internal-error");
...
var TotpMultiFactorAssertionImpl = class TotpMultiFactorAssertionImpl extends MultiFactorAssertionImpl {
	...
	async _finalizeEnroll(auth, idToken, displayName) {
		_assert$4(typeof this.secret !== "undefined", auth, "argument-error");
	...
	async _finalizeSignIn(auth, mfaPendingCredential) {
		_assert$4(this.enrollmentId !== void 0 && this.otp !== void 0, auth, "argument-error");
...
function registerAuth(clientPlatform) {
	_registerComponent(new Component("auth", (container, { options: deps }) => {
		...
		_assert$4(apiKey && !apiKey.includes(":"), "invalid-api-key", { appName: app.name });
...
* limitations under the License.
*/
getExperimentalSetting("authIdTokenMaxAge");
...
//#endregion
//#region node_modules/@firebase/auth/dist/esm/internal.js
/**
* @license
* Copyright 2021 Google LLC
*
...
async function _generateHandlerUrl(auth, event, provider) {
	...
	if (_isIOS()) additionalParams["ibi"] = BuildInfo.packageName;
	else if (_isAndroid()) additionalParams["apn"] = BuildInfo.packageName;
	else _fail(auth, "operation-not-supported-in-this-environment");
	if (BuildInfo.displayName) additionalParams["appDisplayName"] = BuildInfo.displayName;
	additionalParams["sessionId"] = sessionDigest;
	return _getRedirectUrl(auth, provider, event.type, void 0, event.eventId ?? void 0, additionalParams);
}
/**
...
async function _validateOrigin(auth) {
	const { BuildInfo } = _cordovaWindow();
	const request = {};
	if (_isIOS()) request.iosBundleId = BuildInfo.packageName;
	else if (_isAndroid()) request.androidPackageName = BuildInfo.packageName;
	else _fail(auth, "operation-not-supported-in-this-environment");
...
function _performRedirect(handlerUrl) {
	...
	return new Promise((resolve) => {
		cordova.plugins.browsertab.isAvailable((browserTabIsAvailable) => {
			...
			else iabRef = cordova.InAppBrowser.open(handlerUrl, _isIOS7Or8() ? "_blank" : "_system", "location=yes");
			resolve(iabRef);
		});
	});
}
	...
	try {
		await new Promise((resolve, reject) => {
			...
			function authEventSeen() {
				...
				if (typeof iabRef?.close === "function") iabRef.close();
			}
			function resumed() {
			...
			if (_isAndroid()) document.addEventListener("visibilitychange", visibilityChanged, false);
			cleanup = () => {
...
function _checkCordovaConfiguration(auth) {
	...
	_assert$4(typeof win?.universalLinks?.subscribe === "function", auth, "invalid-cordova-configuration", { missingPlugin: "cordova-universal-links-plugin-fix" });
	_assert$4(typeof win?.BuildInfo?.packageName !== "undefined", auth, "invalid-cordova-configuration", { missingPlugin: "cordova-plugin-buildInfo" });
	_assert$4(typeof win?.cordova?.plugins?.browsertab?.openUrl === "function", auth, "invalid-cordova-configuration", { missingPlugin: "cordova-plugin-browsertab" });
	_assert$4(typeof win?.cordova?.plugins?.browsertab?.isAvailable === "function", auth, "invalid-cordova-configuration", { missingPlugin: "cordova-plugin-browsertab" });
	_assert$4(typeof win?.cordova?.InAppBrowser?.open === "function", auth, "invalid-cordova-configuration", { missingPlugin: "cordova-plugin-inappbrowser" });
}
/**
...
async function computeSha256(sessionId) {
	...
	return Array.from(new Uint8Array(buf)).map((num) => num.toString(16).padStart(2, "0")).join("");
}
...
function _eventFromPartialAndUrl(partialEvent, url) {
	...
	if (callbackUrl.includes("/__/auth/callback")) {
		...
		const code = (params["firebaseError"] ? parseJsonOrNull(decodeURIComponent(params["firebaseError"])) : null)?.["code"]?.split("auth/")?.[1];
...
function _getDeepLinkFromCallback(url) {
	...
	return searchParamsOrEmpty(iOSDeepLink)["link"] || iOSDeepLink || doubleDeepLink || link || url;
}
/**
...
var CordovaPopupRedirectResolver = class {
	...
	async _openRedirect(auth, provider, authType, eventId) {
		...
		return _waitForAppResume(auth, manager, await _performRedirect(await _generateHandlerUrl(auth, event, provider)));
	}
	...
	attachCallbackListeners(auth, manager) {
		...
		const universalLinksCb = async (eventData) => {
			...
			let finalEvent = null;
			if (partialEvent && eventData?.["url"]) finalEvent = _eventFromPartialAndUrl(partialEvent, eventData["url"]);
		...
		_cordovaWindow().handleOpenURL = async (url) => {
			if (url.toLowerCase().startsWith(packagePrefix)) universalLinksCb({ url });
...
//#endregion
//#region node_modules/@firebase/auth-compat/dist/index.esm.js
...
function _isWebStorageSupported() {
	try {
		...
		if (storage) {
			...
			if (_isLocalStorageNotSynchronized()) return isIndexedDBAvailable();
			return true;
		}
	} catch (e) {
...
async function _savePersistenceForRedirect(auth) {
	...
	if (session) session.setItem(key, auth._getPersistenceType());
}
function _getPersistencesFromRedirect(apiKey, appName) {
	const session = getSessionStorageIfAvailable();
	if (!session) return [];
	const key = _persistenceKeyName(PERSISTENCE_KEY, apiKey, appName);
	switch (session.getItem(key)) {
		case Persistence.NONE: return [inMemoryPersistence];
		case Persistence.LOCAL: return [indexedDBLocalPersistence, browserSessionPersistence];
		case Persistence.SESSION: return [browserSessionPersistence];
		default: return [];
	}
}
...
function credentialFromObject(object) {
	...
	if (!(object instanceof FirebaseError)) {
		if ("temporaryProof" in _tokenResponse && "phoneNumber" in _tokenResponse) return PhoneAuthProvider$1.credentialFromResult(object);
	}
	const providerId = _tokenResponse.providerId;
	if (!providerId || providerId === ProviderId.PASSWORD) return null;
	let provider;
	switch (providerId) {
		...
		default:
			...
			if (!oauthAccessToken && !oauthTokenSecret && !oauthIdToken && !pendingToken) return null;
			if (pendingToken) if (providerId.startsWith("saml.")) return SAMLAuthCredential._create(providerId, pendingToken);
			else return OAuthCredential._fromParams({
				providerId,
...
function convertCredential(auth, credentialPromise) {
	return credentialPromise.catch((e) => {
		if (e instanceof FirebaseError) attachExtraErrorFields(auth, e);
		throw e;
	}).then((credential) => {
...
var Auth = class {
	constructor(app, provider) {
		...
		_assert$1(apiKey, "invalid-api-key", { appName: app.name });
		_assert$1(apiKey, "invalid-api-key", { appName: app.name });
		const resolver = typeof window !== "undefined" ? CompatPopupRedirectResolver : void 0;
		this._delegate = provider.initialize({ options: {
	...
	async setPersistence(persistence) {
		...
		switch (persistence) {
			...
			case Persistence.LOCAL:
				converted = await _getInstance(indexedDBLocalPersistence)._isAvailable() ? indexedDBLocalPersistence : browserLocalPersistence;
				break;
			case Persistence.NONE:
				converted = inMemoryPersistence;
				break;
			default: return _fail("argument-error", { appName: this._delegate.name });
		}
...
function buildPersistenceHierarchy(apiKey, appName) {
	...
	if (typeof self !== "undefined" && !persistences.includes(indexedDBLocalPersistence)) persistences.push(indexedDBLocalPersistence);
	if (typeof window !== "undefined") {
		for (const persistence of [browserLocalPersistence, browserSessionPersistence]) if (!persistences.includes(persistence)) persistences.push(persistence);
	}
	if (!persistences.includes(inMemoryPersistence)) persistences.push(inMemoryPersistence);
	return persistences;
}
/**
* @license
...
var RecaptchaVerifier = class {
	constructor(container, parameters, app = firebase.app()) {
		_assert(app.options?.apiKey, "invalid-api-key", { appName: app.name });
		this._delegate = new RecaptchaVerifier$1(app.auth(), container, parameters);
...
function registerAuthCompat(instance) {
	instance.INTERNAL.registerComponent(new Component(AUTH_TYPE, (container) => {
		return new Auth(container.getProvider("app-compat").getImmediate(), container.getProvider("auth"));
